...
instance PP [Int] where
  pp = PP.brackets . PP.hcat . PP.punctuate (PP.text ", ") . map PP.int
...
instance PP Var where
  pp (Name x)       = PP.text x
  pp (Proj e index) = pp e <> PP.brackets (pp index) 
...
instance PP Predicate where
  pp (Predicate binds expr) =
    PP.text "forall" <+> PP.hcat (intersperse' (PP.text ", ") (map ppBinding binds)) <+>
    PP.text "::" <+> pp expr
...
instance PP Specification where
  ...
  pp (Ensures pred)   = PP.text "ensures" <+> pp pred <+> PP.char ';'
  pp (Modifies var)   = PP.text "modifies" <+> pp var <+> PP.char ';'
